...
function loadConfiguration() {
  ...
  for (var n = 3; n <= 6; n++)
    pinModesEls('pin' + n, el => el.onclick = jumersADC );

  for (var n = 7; n <= 8; n++)
    pinModesEls('pin' + n, el => el.onclick = jumersUART );
    
  for (var n = 13; n <= 16; n++)
    pinModesEls('pin' + n, el => el.onclick = jumersPWM );
...
function initDefaultVueInstance() {
  return new Vue({
    ...
    methods: {
      resetConfig: () => { 
        if (confirm('Are you sure to clear the configuration?')) 
          window.location.search = ''; 
      },
      copyURL: () => {
        vue.$copyText(window.location.href).then(
          () => showSnack(true, 'Success! Link copied'), 
          () => showSnack(false) 
        ); 
      },
      copyCode: () => { 
        vue.$copyText(code.textContent).then( 
          () => showSnack(true, 'Success! Code copied'),
          () => showSnack(false)
        ); 
      },
      openPage: function (page) { 
        if ((this.pages.active === 1) && !this.relation.count) 
          addRelation();
